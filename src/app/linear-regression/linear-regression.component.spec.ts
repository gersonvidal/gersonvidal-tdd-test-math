...
describe('linear regression test suite', () => {
  ...
  describe('Dataset 3', () => {
    ...
    it('Should return B1 = 1.43097 with the dataset when proxySize = [130,650,99,150,128,302,95,945,368,961] and actualDevelop = [15.0,69.9,6.5,22.4,28.4,65.9,19.4,198.7,38.8,138.2]', async () => {
      const dataset = await readCsvWithColumns(csvFile);
      const result = LinearRegressionComponent.calculateBeta1(dataset);
    ...
    it('Should return yk = 528.4294 with the dataset when proxySize = [130,650,99,150,128,302,95,945,368,961] and actualDevelop = [15.0,69.9,6.5,22.4,28.4,65.9,19.4,198.7,38.8,138.2] and if x = 386', async () => {
      const dataset = await readCsvWithColumns(csvFile);
      const beta1 = LinearRegressionComponent.calculateBeta1(dataset);
      const beta0 = LinearRegressionComponent.calculateBeta0(beta1, dataset);
      const xk = 386;
